...
16. **Группировка отделений по этажам в табе "Oddělení"** (7 октября 2025):
   - Убрана повторяющаяся информация об этаже у каждого отделения (был серый чип "1. patro" над каждым названием)
   - Отделения теперь сгруппированы по этажам - сначала показывается этаж, под ним список всех отделений
   - Дизайн секций этажей: зеленый badge с названием этажа (аналогичен алфавитному разделению в списке рабочих мест)
...
### Группировка отделений по этажам (октябрь 2025):
**ПРОБЛЕМА**: Информация об этаже повторялась у каждого отделения (серый чип "1. patro"), что выглядело избыточно
**РЕШЕНИЕ**:
...
- **Сортировка этажей по номеру**:
  ...
  ```
- **Визуальный дизайн**:
  - Зеленый badge с названием этажа (`.floor-badge`)
  - Стиль аналогичен алфавитному разделению в списке рабочих мест
  - Padding 8x16px, закругленные углы 6px, тень для акцента
  - Цвет: `#60CCAF` (бирюзовый, как в alphabet-badge)
- **Структура HTML**:
  - `.floor-section` - контейнер для всей секции этажа
  - `.floor-header` - заголовок с badge этажа
  - Под заголовком - список отделений без дублирования информации об этаже
- **Преимущества**:
