...
11. **Автоматический зум на выбранное здание** (2 октября 2025):
   ...
   - Плавная анимация зума и панорамирования (600ms с ease-in-out)
   - Автоматический расчет оптимального масштаба с padding вокруг здания
   - Учитывается ширина открытой панели - центр смещается вправо
...
- **Функция `zoomToBuilding(budovaId)`**:
  ...
  - **ПРАВИЛЬНАЯ математика трансформации координат**:
    - Формула: `viewportX = svgX * realZoom + panX`
    - Обратная: `panX = viewportX - svgX * realZoom`
    - Работает корректно при любом текущем положении карты
  - Плавная анимация через `requestAnimationFrame` (600ms)
  - Easing function: ease-in-out cubic для красивой анимации
- **Центрирование**:
  - Панель открыта → карта уже сжата, берем центр её контейнера (`width / 2`)
  - Панель закрыта → тоже центр контейнера
  - Здание ровно посередине доступной области независимо от размера экрана
- **Интеграция**:
...
- **Результат**: Точный зум на здание независимо от размера карты, масштаба и состояния панели
